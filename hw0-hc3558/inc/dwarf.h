...
// Reshape 1D array
//
void reshape_1d_array(float* arr, int cols, int rows, int chans)
{
    // remove padding (-2) and after max pooling (/2)
    cols = (cols - 2) / 2;
    rows = (rows - 2) / 2;


    int len = cols * rows * chans;
    int size = cols * rows;
    int index_flat = 0;
    float* temp_flat = new float[len];

    // re-arrange the data in the array
    for (int j = 0 ; j < size ; j++)
    {
        for (int i = 0 ; i < chans ; i++)
        {
            ...
            temp_flat[index_flat] = arr[index];
            index_flat++;
        }
    }
