...
public:
        ...
        CCS_DESIGN(conv_layer_sysc_catapult)
        CCS_INIT_S1(acc);
        ...
        system_t(sc_module_name name,
            ...
            : sc_module(name), memory(new ac_int<DMA_WIDTH>[MEM_SIZE]), dmac(new esp_dma_controller<DMA_WIDTH, MEM_SIZE>("dma-controller", memory)), driver(new driver_t("driver"))
#if defined(SMALL)
              ,
...
#endif
              ,
              resetn("resetn"), acc_done("acc_done"), model_path(model_path), image_path(image_path)
        {
