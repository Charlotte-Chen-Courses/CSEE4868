...
if [[ $# < 2 ]] ; then
    echo 'usage: ./test1.sh <image> <arch> '
    echo ' e.g.: ./test1.sh cat SMALL'
    exit 1
fi
...
mv test1.txt test/test_native_${arch}_${image}.txt

/usr/bin/diff -q test/test_native_${arch}_${image}.txt golden_fp_tests/test1.txt > /dev/null
...
mv accelerated_test_syn_1.txt test/accelerated_test_beh_${arch}_${image}.txt
...
mv accelerated_test_syn_1.txt test/accelerated_test_rtl_${arch}_${image}.txt
